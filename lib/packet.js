  ...
  if (serverCountFlate > 0) {
    ...
    zlib.inflate(_buff, function (err, buffer) {
      ...
      while (posTemp < buffer.length) {
        ...
        serverList[copyTemp].push({host: buffer.readInt32LE(posTemp), port: buffer.readInt32LE(posTemp + 4)});
        posTemp += 8;
        countTemp++;
...
exports.requestGetOrRemovePacket = function (namespace, key, remove) {
  var keyBuffer = transcoder.encode(key);
  if(!remove)
...
exports.responseGetPacket = function (data) {
  ...
  pos += 4;
  if (vCount > 1) {
    console.log('Tair: warn: this version cannot support multi get');
  }
  ...
  return {key: kBuf.toString('utf-8').replace('\u0000', ''), value: vBuf.toString('utf-8').replace('\u0000', '')};
};
...
function readLongLE(buffer, pos) {
  var _buf = new Buffer(8);
  var _buf_le = new Buffer(8);
  buffer.copy(_buf, 0, pos, pos + 8);
  for (var i = 0; i < 8; i++) {
    _buf_le[ 8 - i ] = _buf[i];
  }
  return utils.decodeLong(_buf_le);
}
