 ...
 * @params callback(err):
 */
var Tair = module.exports = function (groupName, hostList, callback) {
  ...
  var that = this;
  this.requestQueue = [];
  this.retrieveConfigure(groupName, hostList, function (err) {
    if (err) {
      return callback(err);
    }
    callback(null);
...
Tair.prototype.retrieveConfigure = function (groupName, hostList, callback) {
  var that = this;
  configServer.retrieveConfigure.apply(this, [groupName, 0, hostList, function (err) {
    if (err) {
      return callback(err);
    }
 ...
 */
Tair.prototype.get = function (key, namespace, callback, fitJava) {
  var _namespace = typeof namespace === 'number' ? namespace : 0;
  if (typeof namespace === 'function') {
    callback = namespace;
  }
